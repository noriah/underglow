...
// Fill `data` of `size` with `patternSize` colors from `pattern` each color
// getting `blockSize` pixels.
void fillPattern(CRGB *data, pixel_t size, CRGB *pattern, uint8_t patternSize, pixel_t blockSize);

// Fill `data` of `size` with `count` colors from `pattern` each color getting
// relative number from `sizes`.
